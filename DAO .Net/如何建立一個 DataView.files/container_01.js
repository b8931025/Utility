...
*/
//2007/05/14 chingwei (+) 加上icon參數，來顯示各widget的圖
...
// 2007/4/16 chingwei (+) 增加show and hide EDIT(目前沒有加event，所以沒作用)
...
YAHOO.widget.Module.prototype = {
   ...
   */ 
	...
	*/
	...
	edit : null,	// 2007/04/16 chingwei (+) BGN 新增EDIT區塊
  ...
  init : function(el, userConfig, icon) {	  
	...
	*/
    ...
    // 2007/04/16 chingwei (+) END 設定EDIT區塊	
	...
	*/
	render : function(appendToNode, moduleElement) {
		...
		// 2007/04/16 chingwei (+) BGN 新增EDIT區塊
    ...
    //2007/5/14 chingwei(+) 增加Header的功能圖示
...
*/    
  if(this.widgetIcon != undefined){  
    var showEditObj = document.createElement("img");
		showEditObj.id = this.id+'_showEdit'; //wesley:須避免id重複,故串上此module的id(this.id).
    ...
    showEditDiv.appendChild(showEditObj);
		...
		expandObj.id = this.id+'_expand'; //wesley:須避免id重複,故串上此module的id(this.id).
    ...
    expandDiv.appendChild(expandObj);
		...
		destroyObj.id = this.id+'_destroy'; //wesley:須避免id重複,故串上此module的id(this.id).
    ...
    destroyDiv.appendChild(destroyObj);
		...
		iconObj.id = this.id+'_icon'; //wesley:須避免id重複,故串上此module的id(this.id).
...
/**wesley: 暫不需要
	// 2007/4/16 chingwei (+) BGN 隱藏EDIT區塊
	hideEdit : function() {
		YAHOO.util.Dom.setStyle(this.edit, "display", "none");
		//this.cfg.setProperty("visible", false);
	},
	// 2007/4/16 chingwei (+) END 隱藏EDIT區塊
	// 2007/4/16 chingwei (+) BGN 顯示EDIT區塊
	showEdit : function() {
		YAHOO.util.Dom.setStyle(this.edit, "display", "block");
		//this.cfg.setProperty("visible", true);
	},
	// 2007/4/16 chingwei (+) END 顯示EDIT區塊
*/
	// wesley:顯示及隱藏EDIT區塊 BGN>>
	showEdit : function() {
	...
	expand : function() {
		if(YAHOO.util.Dom.getStyle(this.body, 'display')=="none") {
		  ...
		  YAHOO.util.Dom.get(this.id+'_expand').className = 'icon widget-collapse';   //chingwei (+) 改成設定css的方式
		} else {
		  //alert("test:"+YAHOO.util.Dom.getStyle(this.edit, 'display'));
		  YAHOO.util.Dom.setStyle(this.edit, "display", "none");
		  ...
		  YAHOO.util.Dom.get(this.id+'_expand').className = 'icon widget-expand';   //chingwei (+) 改成設定css的方式
		}
	},
	// wesley:縮放區塊 END<<
