...
function Calendar (cname, id, date, bid)
{
  ...
  this.bid = bid;
	...
	this.calendarStyle = "calMonth";
	this.headerStyle = "calMonthCurrent";
	this.headerCellStyle = "calMonthHeader";
	this.headerCellStyleLabel = "calMonthHeader";
...
function getCurrentYYYYMM(calendar){
  return calendar.calendarDate.getUTCFullYear() + '' + ( calendar.calendarDate.getUTCMonth() < 9 ? '0'+(calendar.calendarDate.getUTCMonth()+1) : calendar.calendarDate.getUTCMonth()+1 );
}
...
{ 
	...
	/* add by garlic 2006.11.06 */
	/*connect('/_theme/ArticleCalendarExp.php?yyyymm='+getCurrentYYYYMM(calendar)+'&bid=' + calendar.bid,'xxx');*/
	$.ajax({
            type: "GET",
            url: '/_theme/ArticleCalendarExp.php?yyyymm='+getCurrentYYYYMM(calendar)+'&bid=' + calendar.bid,
            dataType: 'json',
            success: xxx
          });
}
...
function RenderDayEvent (calendar, currentDate, dayStyle, eventId)
{
	currentDayStyle = dayStyle;
	calHtml += ('<td class="day">');
	calHtml += ("<span class=\"" + dayStyle + "\" onclick=\" " + calendar.name + ".selectEvent('" + eventId + "')\">");
	calHtml += (currentDate.getUTCDate());
	calHtml += ("</span>");
	calHtml += ("</td>");
}
...
function buildCalendarTable (calendar)
{
	...
	odd = 0;
	/* patched by misgarlic @ 2007.03.29 :: today event do not been rendering */
	mark_today = 0;
			...
			else if (currentDate.getUTCDate() == calendar.calendarDate.getUTCDate())
			{
				if ((currentDate.getUTCDay() == 0) || (currentDate.getUTCDay() == 6))
				{
					...
					if (calendar.allowWeekdays == true)
					{
  					...
  					if(mark_today == 0)
						  RenderDayEnabled (calendar, currentDate, calendar.todayDayStyle);
            /* end patched by misgarlic @ 2007.03.29 :: today event do not been rendering */  					
					} 
					else 
					{
						//RenderDayDisabled (calendar, currentDate);	
